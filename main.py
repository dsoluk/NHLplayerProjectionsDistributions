...
from nhlproj.scoring.scoring import compute_category_scores, scale_scores
...
def main():
    ...
    if scope != "ALL":
        if "pos_group" not in df.columns:
            ...
            if scope == "F":
                mask = df["pos_group"].isin(["F", "B"]).fillna(False)
            else:  # scope == "D"
                mask = df["pos_group"].isin(["D", "B"]).fillna(False)
            kept = int(mask.sum())
